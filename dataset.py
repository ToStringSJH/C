...
# def load_data(dataset_name, show_details=True):
...
def FeatureNormalize(mx):
    ...
    # 将结果转回PyTorch张量并放回原设备
    return torch.from_numpy(mx).float().to(device)
# def FeatureNormalize(mx):
#     """Row-normalize sparse matrix"""
#     rowsum = np.array(mx.sum(1))
#     rowsum = (rowsum == 0) * 1 + rowsum              #!!!!! 
...
def cal_norm(edge_index0, args, feat=None, cut=False, num_nodes=None):
    # calculate normalization factors: (2*D)^{-1/2} or (D)^{-1/2}
    if isinstance(edge_index0, torch.Tensor):
        if edge_index0.is_sparse:
            edge_index0 = edge_index0.to_dense()  # 先将稀疏张量转换为密集张量
        edge_index0 = edge_index0.detach().cpu().numpy()
    edge_index0 = sp.coo_matrix(edge_index0)
    values = edge_index0.data  
    indices = np.vstack((edge_index0.row, edge_index0.col))
    edge_index0 = torch.LongTensor(indices).to(args.device) 
...
# def cal_norm(edge_index0, args, feat=None, cut=False, num_nodes=None):
#     # calculate normalization factors: (2*D)^{-1/2} or (D)^{-1/2}
#     if isinstance(edge_index0, torch.Tensor) and edge_index0.is_sparse:
#         row = edge_index0.indices()[0].cpu().numpy()
#         col = edge_index0.indices()[1].cpu().numpy()
#         values = edge_index0.values().cpu().numpy()
#         edge_index0 = sp.coo_matrix((values, (row, col)), shape=(args.N, args.N))
#     else:
#         edge_index0 = sp.coo_matrix(edge_index0)
